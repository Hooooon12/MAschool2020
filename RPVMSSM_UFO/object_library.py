...
class Coupling(UFOBaseClass):
    ...
    coupling_class = all_couplings

    def __init__(self, name, value, order, **opt):

        args =(name, value, order)	
        UFOBaseClass.__init__(self, *args, **opt)
        self.coupling_class.append(self)
...
class CTCoupling(Coupling):
    global all_CTcouplings
    coupling_class = all_CTcouplings
...
class Restriction(UFOBaseClass):

    require_args = ['name', 'restrictions']
    require_args_all = ['name', 'restrictions', 'default']

    def __init__(self, name, restrictions, default=None, **opt):
        args = (name, restrictions, default)
        UFOBaseClass.__init__(self, *args, **opt)

        global all_restrictions
        all_restrictions.append(self)

all_assumptions = []

class Assumption(UFOBaseClass):

    require_args = ['name', 'condition']
    require_args_all = ['name', 'condition', 'validity']
    
    def __init__(self, name, condition, validity="all", **opt):
        args = (name, condition, validity)
        UFOBaseClass.__init__(self, *args, **opt)

        global all_assumptions
        all_assumptions.append(self)
